// +------------------------------------------------------------------+
// | RIRISCALPS v1.0 — MARKET BUILD (no DLL/WebRequest)                |
...
#property tester_indicators "MA,RSI,ATR"
#property tester_symbols "XAUUSD"
...
#define GA_ROOT 2
// [MARKET-BUILD] WinAPI dibuang (prohibited: terminal manipulation)
...
input int MaxOpenOrders = 4;                              // Max Consecutive Open Orders
...
input double HedgeMaxLot = 0.05;                          // Hard Lot Ceiling Per Hedge Leg
...
input double MaxLotSize = 0.03;                           // Max Lot Size

input group "🏦 Equity Settings"
input bool EnableBasketStop = true;                       // Close All When Total Floating Loss Exceeds Limit
input double MaxBasketLossPct = 6.0;                      // Max Total Floating Loss as % of Equity (0 = Disabled)
...
input double MinimumEquity = 100;                          // Min Equity - Stop Trading when Reached (0 = Disabled)
...
input int MagicNumber = 8642000;                          // Magic Number
...
bool CreatePasswordDialog()
{
    ...
    passwordDialog.Caption("Enter Password to Use RiriScalps EA");
...
int InitializeEA()
{
    ...
    Print("+-----------------------------------------+");
    Print("RiriScalps v1.0 Initialized Successfully");
    Print("+-----------------------------------------+");
...
{   
    ...
    Print("RiriScalps v1.0 Deinitialized");
}
...
void OnTick()
{
    ...
    {   
        ...
        CloseAllPositions();
        // [MARKET-BUILD] auto-disable algo dihapus — user kontrol penuh
...
{   
    ...
    request.comment = "Open Position by RiriScalps";
    request.type_filling = GetFillingMode();
...
ulong OpenChainHedge(ulong chainId, ENUM_POSITION_TYPE prevType, double hedgeLot, int newLevel, double anchorLoss, int cycleNum)
{
    ...
    request.deviation = 10;
    request.magic = MagicNumber;
    request.comment = "Hedge L" + IntegerToString(newLevel) + " by RiriScalps";
    request.type_filling = GetFillingMode();
...
void PlaceLimitEntry(ENUM_ORDER_TYPE dir, double signalScore)
{
    ...
    request.comment = "RiriLE|" + DoubleToString(signalScore, 2); // stash entry-thesis score
    request.price = entry;
...
{
    int p = StringFind(comment, "RiriLE|");
    if(p < 0) return -1;
...
// [MARKET-BUILD] DisableAlgoTrading dihapus (PostMessageW prohibited)

// +------------------------------------------------------------------+
// | Send Discord alert via webhook                                   |
// +------------------------------------------------------------------+
bool SendDiscordAlert(string title, string message, int embedColor = 3447003){ return(false); } // [MARKET-BUILD] WebRequest dibuang
